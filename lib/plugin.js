...
class Plugin {
  ...
  apply(compiler) {
    if (compiler.hooks) {
      ...
      compiler.hooks.compilation.tap('MangleJsClassPluginHooks', compilation => {
        compilation.hooks.optimizeChunkAssets.tapAsync('MangleJsClassPluginOptimizeChunkAssetsHooks', runner(compiler, compilation, this.opts));
      });
    } else {
